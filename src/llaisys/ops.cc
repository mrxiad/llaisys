...
__C {
    ...
    void llaisysLinear(llaisysTensor_t out, llaisysTensor_t in, llaisysTensor_t weight, llaisysTensor_t bias) {
        llaisys::ops::linear(out->tensor, in->tensor, weight->tensor, bias == nullptr ? nullptr : bias->tensor);
    }
    ...
    void llaisysSample(llaisysTensor_t sampled_idx, llaisysTensor_t logits, float temperature, int top_k, float top_p) {
        llaisys::ops::sample(sampled_idx->tensor, logits->tensor, temperature, top_k, top_p);
    }
}
